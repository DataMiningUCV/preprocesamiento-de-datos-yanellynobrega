...
for row in data:
    ...
    minable_obj.writerow(np.concatenate((row[0:3], row[4:11], row[13:21], row[22:26], row[29:30], row[31:32], row[33:37],row[38:47], row[48:52], row[53:62], row[63:65]), axis=0))
...
# Open up the csv file in to a Python object
dataO = open('minable.csv', 'rb')
data_file_object = csv.reader(dataO)
header = data_file_object.next()  # The next() command just skips the 
                                 
data1=[]                    # Create a variable called 'data'.
for row in data_file_object:
      # Run through each row in the csv file,
    data1.append(row[1:])             # adding each row to the data variable
